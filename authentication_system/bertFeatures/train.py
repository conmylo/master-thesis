...
# Extract stylometric features (without time of day)
def extract_features(text):
    features = {}
    words = nltk.word_tokenize(text)
    char_count = len(text) if len(text) > 0 else 1  # Avoid division by zero

    # Existing stylometric features...
    ...
    stylometric_features = list(features.values())

    # Get BERT embedding for the text and concatenate with stylometric features
    embedding = get_bert_embedding(text)
    combined_features = np.concatenate((embedding, stylometric_features))
